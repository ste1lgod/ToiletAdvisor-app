 ...
 *   1. npm install                  (один раз — устанавливает firebase-admin)
 *   2. Скопируй serviceAccountKey.json в корень проекта:
...
// serviceAccountKey.json лежит в корне проекта (на уровень выше scripts/)
let serviceAccount;
try {
  serviceAccount = require(path.join(__dirname, '..', 'serviceAccountKey.json'));
} catch (e) {
  console.error('❌ Файл serviceAccountKey.json не найден в корне проекта!');
...
const SEED_TOILETS = [
  { id:'seed-0001-tashkent-city-mall', lat:41.2956, lon:69.2797,
    addr:'проспект Ислама Каримова, 1',
  ...
  { id:'seed-0002-masjid-minor', lat:41.2871, lon:69.2768,
    addr:'улица Катартал, 56',
  ...
  { id:'seed-0003-railway-station', lat:41.3174, lon:69.2870,
    addr:'улица Мустакиллик, 1',
  ...
  { id:'seed-0004-navoi-park', lat:41.3058, lon:69.2714,
    addr:'улица Алишера Навои, 30',
  ...
  { id:'seed-0005-khastimom', lat:41.3276, lon:69.2623,
    addr:'улица Зайнабидин Восиков, 103',
    title:'Тахаратхана в мечети Хастимом',
    description:'Тахаратхана при соборной мечети Хасти-Имом, рядом с медресе',
    isOpen:true,  isFree:true,  hasSoap:true,  hasPaper:false, isAccessible:false, isTaharatkhana:true  },
  { id:'seed-0006-next-mall', lat:41.3376, lon:69.2841,
    addr:'проспект Амира Темура, 108',
    title:'Туалет в ТРЦ Next',
    description:'Туалет на 2-м этаже торгового центра Next на Юнусабаде',
    isOpen:true,  isFree:true,  hasSoap:true,  hasPaper:true,  isAccessible:true,  isTaharatkhana:false },
  { id:'seed-0007-broadway', lat:41.2983, lon:69.2717,
    addr:'улица Сайилгох (Broadway)',
...
async function main() {
  ...
  await batch.commit();
  console.log(`   ✅ Записано ${SEED_TOILETS.length} туалетов (с адресами)`);
